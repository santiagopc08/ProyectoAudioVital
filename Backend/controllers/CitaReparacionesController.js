...
exports.crearCita = (req, res) => {
    ...
    const cita = new CitaReparaciones(req.body)
    ...
    Paciente.findById(cedula).exec((err, paciente) => {
        if (paciente == undefined || paciente == null) {
            ...
            CitaReparaciones.find({hora: horaP, fecha: fechaP }).exec((e, citas) => {
                if (err) {
...
exports.traerCitas = (req, res) => {
    ...
    CitaReparaciones.find().exec(function (err, cita) {
        utils.show(res, err, cita)
    })
}
...
exports.citasAgendadasPorDia = (req, res) => {
    ...
    CitaReparaciones.find({fecha: fechaP}).exec((err, result) => {
        if(err) throw err
...
exports.citaPacientesAgendadaPorDia = async (req, res) => {
    ...
    await CitaReparaciones.find({fecha: fechaP}).exec().then(result => {
        citas = result
    }).catch(err => {
